...
export const Config = z.object({
  ...
  /**
   * Whether the client asks for a second confirmation before applying. Off by
   * default: the editor is already an explicit action the user opened, and the
   * panel states what saving does. Set it to `true` to require the extra step.
   */
...
export function readMessageText(event) {
  ...
  // types nest them under `data.message`. Both shapes have to read the same way
  // now that the model's replies are editable too.
...
export function rollbackLedger(events) {
  ...
  for (const replacement of folded.replacements) {
    ...
    if (!(source && (source.kind === `plugin:${PLUGIN_ID}` || (source.kind === 'plugin' && source.plugin === PLUGIN_ID)))) continue
...
export function planRollback(events, surfaceNodes, request) {
  ...
  // Two editable kinds, two outcomes: rewriting a prompt re-runs the turn;
  // rewriting a reply replaces the answer and lets the conversation continue.
...
async function stateOf(ctx, sessionId, config) {
  ...
  const live = findLiveSession(ctx, sessionId)
  const turns = editableTurns(events, folded.nodes)
 ...
 *
 * @param plan - the planned window, for the original's turn/step/provider.
 * @param text - the replacement text.
 * @returns `{ type, data }` for `session.append`, to be marked `surfaceOp: 'append'`.
 */
export function buildCorrection(plan, text) {
  ...
  return {
    ...
    data: {
      turn: typeof plan.turn === 'number' ? plan.turn : 0,
      step: typeof plan.step === 'number' ? plan.step : 1,
      message: {
        id: randomUUID(),
        role: 'assistant',
...
async function applyEdit(ctx, sessionId, body, config) {
  ...
  // not asked again.
  if (plan.mode === 'reply') {
    const correction = buildCorrection(plan, text)
    let appended
    try {
      appended = session.append(correction.type, correction.data, { surfaceOp: 'append' })
      // Close the turn again, now that the corrected reply is the last thing in
      // it. The host anchors a turn's tail (its duration and action strip) at the
      // turn's LAST `turn/end`, so a correction appended after the old one lands
      // OUTSIDE the turn and the tail renders above the corrected text. These two
      // events put the tail back behind it, exactly as a normal turn ends.
      session.append('step/end', { turn: appended.data.turn, step: appended.data.step })
      session.append('turn/end', { turn: appended.data.turn, reason: { kind: 'completed' } })
    } catch (error) {
      return {
        kind: 'reply',
        replacementSeq: replacement.seq,
  ...
  // A rewritten prompt is re-run through the only official prompt path, which
  // also resumes a cold Session and starts exactly one new turn.
  const controller = ctx.get('sessionController')
  ...
  tools.register(
    defineTool({
      ...
      async execute(args, exec) {
        ...
        lines.push('editing a turn re-runs it; editing a reply replaces its text and discards what follows.')
        return lines.join('\n')
      },
    }),
  )
