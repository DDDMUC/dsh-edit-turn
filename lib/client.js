...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
        ...
        const pending = fetch(url, { headers: { accept: 'application/json' } })
          .then(async (res) => {
            ...
            for (const reply of Array.isArray(data.replies) ? data.replies : []) {
              if (reply && typeof reply.seq === 'number') replies.set(reply.seq, reply)
            }
            this.publish({
              hidden,
              editable,
    ...
    //
    // Two kinds are offered: a human prompt (rewritten, then the turn re-runs)
    // and a model reply (rewritten in place, discarding everything after it).
    // A step row can span several surface nodes, and the reply seq sits on
    // `finalNode` for an assistant step while a user row carries it on `data`,
    // so both candidates are tried rather than assumed.
    function targetFor(node, view) {
      ...
      if (node.kind === 'user') {
        ...
        push(data.finalNode && data.finalNode.seq)
        push(node.anchorSeq)
      } else {
        return null
      }
      for (const seq of candidates) {
        ...
        const entry = view.editable.get(seq) || view.replies.get(seq)
        if (!entry) continue
        return {
          seq,
          mode: view.editable.has(seq) ? 'prompt' : 'reply',
          turn: entry.turn,
