...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function renderEditor(row, controller, view, t) {
      ...
      if (existing && existing.dataset.dshetFor === String(target.seq)) return

      clearEditor()
      const box = document.createElement('div')
      box.className = 'dshet-editor'
      box.dataset.dshetFor = String(target.seq)
      ...
      if (typeof view.failure === 'string' && view.failure !== null) {
        const error = document.createElement('p')
        error.className = 'dshet-error'
        error.setAttribute('role', 'status')
        error.textContent = t(`error.${view.failure}`)
