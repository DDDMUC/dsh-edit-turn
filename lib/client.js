...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function applyDom(snapshot, view, controller, t) {
      ...
      // editable set may have moved on: the re-run appends a new prompt, which
      // the host will happily let you edit but this half has never asked about.
      // Without this refresh the plugin only ever worked once per page load -
      // the second edit had no action to click because the editable set was
      // still the one fetched at mount time.
      if (rows.length !== controller.rowCount) {
        controller.rowCount = rows.length
