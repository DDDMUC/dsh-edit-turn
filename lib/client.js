    ...
    function applyDom(snapshot, view, controller, t) {
      ...
      for (const row of rows) {
        ...
        if (target !== null) {
          ...
          // The turn tail's own strip is the LAST direct child of the row root.
          // The tail can also hold tool-call rows that carry action bars of
          // their own: those unmount as a call expands, so a pencil parked there
          // blinks in and out, and it is not where the reply's actions live.
          const bar =
            covered === undefined
              ? undefined
