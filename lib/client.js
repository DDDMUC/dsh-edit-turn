...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function injectRowAction(row, target, controller, t, fallback = false) {
      ...
      if (!entry) {
        ...
        if (fallback) host.dataset.dshetFallback = '1'
    ...
    function applyDom(snapshot, view, controller, t) {
      ...
      for (const row of rows) {
        ...
        // offered through this row's action bar, and only while the strip does
        // not already carry one. Both land in the platform's own bar, so the
        // position is identical either way.
        let effective = target
        if (effective === null && !hidden && node.kind === 'turn-tail') {
          const bar = row.querySelector('[class*="_actions"]')
          const slotPencil = bar === null ? null : bar.querySelector('.dshet-row-action:not([data-dshet-fallback])')
          const covered = seqsFor(node).find((seq) => view.replies.has(seq) && !view.hidden.has(seq))
          if (bar !== null && covered !== undefined && slotPencil === null) {
        ...
        if (effective !== null) injectRowAction(row, effective, controller, t, target === null)
